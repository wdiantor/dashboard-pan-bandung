...
async function initDashboard() {
    try {
        ...
        // Pembersihan data dilakukan di awal
        ...
        // PENTING: Jalankan Map dulu baru Filter agar geojsonLayer sudah terisi
        initMap();
...
function initMap() {
    ...
    // Basemap Terang agar kontras dengan biru
    ...
    fetch('kab-bandung.json')
        ...
        .then(geoData => {
            ...
            // Jalankan filter pertama kali SETELAH json peta masuk
            applyFilter();
        });
}
...
function renderChart(labels, dprtData, kaderData) {
    ...
    myChart = new Chart(ctx, {
        ...
        data: {
            labels: labels,
            datasets: [
                { label: 'Kader', data: kaderData, backgroundColor: '#f97316', borderRadius: 4, barThickness: 8 },
                { label: 'DPRT Aktif', data: dprtData, backgroundColor: '#0054a6', borderRadius: 4, barThickness: 8 }
            ]
        },
        options: {
            indexAxis: 'y',
            ...
            scales: {
                y: { ticks: { color: '#1e293b', font: { size: 10, weight: 'bold' } } },
