...
function initMap() {
    ...
    fetch('kab-bandung.json')
        ...
        .catch(err => console.warn("File kab-bandung.json tidak terbaca. Pastikan file ada di folder yang sama."));
}
...
function styleMap(feature) {
    ...
    return {
        fillColor: totalDPRT > 500 ? '#1e3a8a' : totalDPRT > 100 ? '#3b82f6' : '#93c5fd',
        weight: 1,
        color: 'white',
        fillOpacity: 0.7
    };
...
function applyFilter() {
    ...
    const filtered = filterValue === "ALL" ? allData : allData.filter(row => row[2] === filterValue);
    ...
    if (filterValue === "ALL") {
        const sum = {};
        filtered.forEach(row => { if(row[2]) sum[row[2]] = (sum[row[2]] || 0) + (parseInt(row[3]) || 0); });
